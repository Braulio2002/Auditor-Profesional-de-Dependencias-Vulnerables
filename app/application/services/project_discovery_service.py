...
class ProjectDiscoveryService:
    ...
    def discover_projects(self) -> list[ProjectScanTarget]:
        ...
        if not subdirs:
            ...
            if root_files:
                target = self._build_scan_target("Proyecto Raíz", self.settings.datos_entrada_dir, root_files)
                projects.append(target)
                logger.info(f"Proyecto detectado: Proyecto Raíz (en datos_entrada/) con {len(root_files)} archivos.")
        else:
    ...
    def _build_scan_target(self, name: str, path: Path, files: list[Path]) -> ProjectScanTarget:
        ...
        return ProjectScanTarget(name=name, path=path, detected_ecosystems=detected_ecosystems, dependency_files=files)
