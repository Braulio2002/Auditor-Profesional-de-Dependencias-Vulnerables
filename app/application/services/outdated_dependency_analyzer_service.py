...
class OutdatedDependencyAnalyzerService:
    ...
    def analyze_outdated(
        self, dependency: Dependency, latest_version: str | None
    ) -> dict[str, Any] | None:
        """Compara la versión instalada con la última versión recomendada."""
        if (
            not latest_version
            or latest_version == "0.0.0"
            or dependency.installed_version == "0.0.0"
        ):
            return None
        ...
        if lat_major > inst_major:
            ...
            update_risk = "HIGH"
            action = f"Actualización mayor de v{installed} a v{latest}. Puede contener breaking changes. Requiere revisión exhaustiva y pruebas completas de integración."
        elif lat_minor > inst_minor:
            update_type = UpdateType.MINOR
            update_risk = "MEDIUM"
            action = f"Actualización menor de v{installed} a v{latest}. Típicamente añade funcionalidad sin romper compatibilidad. Se recomienda probar en entorno local/staging antes."
        elif lat_patch > inst_patch:
            update_type = UpdateType.PATCH
            update_risk = "LOW"
            action = f"Actualización de parche de v{installed} a v{latest}. Corrige errores o problemas menores. Bajo riesgo. Recomendado aplicar a la brevedad."
        else:
