...
class DependencyReaderFactory:
    ...
    def get_reader(file_path: Path) -> DependencyFileReaderInterface:
        ...
        elif filename in ("requirements.txt", "pyproject.toml", "poetry.lock") or filename.endswith(
            ".txt"
        ):
        ...
        raise ReaderException(f"No existe un lector configurado para el archivo: {filename}")
